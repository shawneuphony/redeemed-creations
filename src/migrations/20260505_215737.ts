...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`CREATE TABLE \`homepage_hero\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`site_footer_columns_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` text NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  	\`href\` text NOT NULL,
  	\`open_in_new_tab\` integer DEFAULT false,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`site_footer_columns\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`site_footer_columns_links_order_idx\` ON \`site_footer_columns_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`site_footer_columns_links_parent_id_idx\` ON \`site_footer_columns_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`site_footer_columns\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`heading\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`site_footer\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`site_footer_columns_order_idx\` ON \`site_footer_columns\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`site_footer_columns_parent_id_idx\` ON \`site_footer_columns\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`site_footer_socials\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`site_footer_socials_order_idx\` ON \`site_footer_socials\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`site_footer_socials_parent_id_idx\` ON \`site_footer_socials\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`site_footer_bottom_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`site_footer_bottom_links_order_idx\` ON \`site_footer_bottom_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`site_footer_bottom_links_parent_id_idx\` ON \`site_footer_bottom_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`site_footer\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`brand_name\` text DEFAULT 'Storefront',
  	\`tagline\` text DEFAULT 'A living catalog of products made for real life.',
  	\`copyright_text\` text DEFAULT 'All rights reserved.',
  	\`updated_at\` text,
  	\`created_at\` text
  );
  `)
  await db.run(sql`CREATE TABLE \`about_page_mission_pillars\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`about_page_mission_pillars_order_idx\` ON \`about_page_mission_pillars\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`about_page_mission_pillars_parent_id_idx\` ON \`about_page_mission_pillars\` (\`_parent_id\`);`)
  ...
  await db.run(sql`CREATE TABLE \`about_page\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`hero_eyebrow\` text DEFAULT 'Our story',
  	\`hero_headline\` text DEFAULT 'We curate what matters.' NOT NULL,
  	\`hero_subtext\` text DEFAULT 'We started with a simple belief — that great products shouldn''t be hard to find. So we built a catalog that does the searching for you.',
  	\`hero_image_id\` integer,
  	\`mission_label\` text DEFAULT 'What we believe',
  	\`mission_statement\` text DEFAULT 'Our mission is to connect people with products that are made thoughtfully, priced fairly, and built to last.',
  	\`team_label\` text DEFAULT 'The people behind it',
  	\`team_headline\` text DEFAULT 'Built by a small team with big taste.',
  	\`cta_headline\` text DEFAULT 'Ready to explore the catalog?',
  	\`cta_subtext\` text DEFAULT 'Browse products curated for real life.',
  	\`cta_button_label\` text DEFAULT 'Shop now',
  	\`cta_button_href\` text DEFAULT '/shop',
  	\`updated_at\` text,
  	\`created_at\` text,
  	FOREIGN KEY (\`hero_image_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  ...
  await db.run(sql`CREATE TABLE \`site_settings\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`whatsapp_number\` text DEFAULT '26771234567' NOT NULL,
  	\`contact_email\` text,
  	\`contact_phone\` text,
  	\`address\` text,
  	\`site_title\` text DEFAULT 'Storefront',
  	\`site_description\` text DEFAULT 'A living catalog of products made for real life.',
  	\`updated_at\` text,
  	\`created_at\` text
  );
  `)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`site_footer_columns_links\`;`)
  await db.run(sql`DROP TABLE \`site_footer_columns\`;`)
  await db.run(sql`DROP TABLE \`site_footer_socials\`;`)
  await db.run(sql`DROP TABLE \`site_footer_bottom_links\`;`)
  await db.run(sql`DROP TABLE \`site_footer\`;`)
  await db.run(sql`DROP TABLE \`about_page_mission_pillars\`;`)
